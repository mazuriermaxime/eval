...
select sum((ode_unit_price-(ode_unit_price*ode_discount/100))*ode_quantity) as chiffre_aff 
FROM orders_details 
JOIN orders 
on orders_details.ode_ord_id=orders.ord_id 
WHERE YEAR(ord_order_date)=2020;
...
select (sum((ode_unit_price-(ode_unit_price*ode_discount/100))*ode_quantity))/count(DISTINCT(ord_id)) as prix_panier 
FROM orders_details 
